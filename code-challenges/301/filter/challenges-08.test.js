...
const notInFirstArray = (forbiddenValues, arr) => {
  let uniqueVals = arr.filter((value) => {
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  ...
  let goodStatNames = arr.filter((stats) => {
    return (stats.baseStat > minBaseStat);
  });
  return goodStatNames;
};
...
const getStatName = (arr, minBaseStat) => {
  let goodStats = arr.filter((stats) => {
    return (stats.baseStat > minBaseStat);
  });

  let names = [];
  goodStats.forEach( (statObject) => {
    names.push(statObject.stat.name);
  });
...
const getCharactersWithoutChildren = (arr) => {
  let noKidsArr = arr.filter((characters) => {
    let objKeys = Object.keys(characters);
    return !(objKeys.includes('children'));
  });
  return noKidsArr;
};

...
const totalSum = (input) => {
  let totalCount = 0;
  input.forEach( arr => {
    const arrSum = arr.reduce( (accum, value) => {
      return accum + value;
    }, 0);
    totalCount += arrSum;
  });
...
//   const finalArr = input.map( arr => {
//     if(arr){
//       let spliceIdxArr = [];
//       let innerArr = arr.map( (value, idx) => {
//         if(value % 5){
//           spliceIdxArr.push(idx);
//         } else {
...
//       });
  ...
  //       for(let i = 0; i < spliceIdxArr.length; i++){
  //         innerArr.splice(spliceIdxArr[i],1);
  //       }

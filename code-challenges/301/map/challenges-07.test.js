...
const extractAbilities = (arr) => {
  let abilitiesArr = arr.map(stats => {
    return stats.ability.name;
  });
  return abilitiesArr;
};
...
const extractStats = (arr) => {
  let statsArr = arr.map(stats => {
    let statCollection = {
      name: stats.stat.name,
      total: stats.effort + stats.baseStat
    };
